...
class Server:
	...
	def register_handler(self, path, handler, full_match=True):
		...
		if full_match:
			path = f"^{path}$"
		self.routes.append({"type": "handler", "regex": re.compile(path), "handler": handler})
	def internal_redirect(self, path, replacer, full_match=True):
		print("register redirect")
		if full_match:
			path = f"^{path}$"
		self.routes.append({"type": "redirect", "regex": re.compile(path), "replacer": replacer})
	...
	def handler_404(self, req):
		...
		with open("templates/404.mustache", "r") as f:
			...
			res.status = 404
			res.headers["Content-Type"] = f"text/html; charset=UTF-8"
		return res
